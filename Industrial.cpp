...
//      currentWorkers: passed by reference to decrement by 2 when pop of 'I' increases
//      futureGoods: passed by reference to increment after pop increase
...
//  Purpose:
//      Manipulates cityGrid to adjust Industrial population, available worker
...
int Industrial::main(vector<vector<CityNode>>& cityGrid, int& futureWorkers, int& currentWorkers,int& futureGoods, int rowLength, int columnLength)
{
    int popChange = 1;
    int changes = 0;
    set<int> grow;
    ...
    for (int y = 0; y < columnLength; y++)
    {
        ...
        {
            if (x >= 0 && x < rowLength && y >= 0 && y < columnLength && cityGrid[y][x].getType() == 'I' && cityGrid[y][x].getCurrentPopulation() < 3)
            {
                if (cityGrid[y][x].getCurrentPopulation() < 1)
                {
                    if (checkForPowerline(cityGrid, x, y, rowLength, columnLength))
                    {
                        if (currentWorkers >= 2)//sorting through if statment trying to align table and get correct takeaway value 
                        {
      ...
      for (auto it = grow.rbegin(); it != grow.rend(); it++)
      {
        for (int y = 0; y < columnLength; y++)
        {
          for (int x = 0; x < rowLength; x++)
          {
            if (x >= 0 && x < rowLength && y >= 0 && y < columnLength && cityGrid[y][x].getType() == 'I' && cityGrid[y][x].getCurrentPopulation() < 3 && currentWorkers >= 2)
            {
...
bool Industrial::checkForPowerline(vector<vector<CityNode>> cityGrid, int x, int y, int row, int column)
{
...
int Industrial::checkAdjacentPopulation(vector<vector<CityNode>> cityGrid, int x, int y, int row, int column)
{
...
int Industrial::determinePriority(vector<vector<CityNode>> cityGrid, int x, int y, int row, int column)
{
